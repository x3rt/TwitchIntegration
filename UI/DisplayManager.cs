...
namespace TwitchIntegration.UI
{
    public static class DisplayManager
    {
        ...
        public static void SetDisplay(int display)
        {
            ...
            {
                Main.loggerInstance?.Warning(
                    $"Cannot set display index to {display} as there are not enough monitors connected!");
            ...
            if (!Camera.main || Camera.main.targetDisplay != display)
            {
                if (!canvasCamera)
                {
                    canvasCamera = new GameObject("TwitchIntegration_CanvasCamera").AddComponent<Camera>();
        ...
        private static IEnumerator FixPanels()
        {
            ...
            foreach (Panels.TIPanel panel in UIManager.UIPanels.Values)
            {
