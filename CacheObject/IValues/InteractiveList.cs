...
namespace TwitchIntegration.CacheObject.IValues
{
    public class InteractiveList : InteractiveValue, ICellPoolDataSource<CacheListEntryCell>, ICacheObjectController
    {
        ...
        public override void SetLayout()
        {
            ...
            this.scrollLayout.minHeight = minHeight;
        }
        ...
        private void CheckGenericIList(object value)
        {
            try
            {
                ...
                if (IsWritableGenericIList)
                {
                    ...
                    {
                        Main.LogWarning($"Failed to find indexer property for IList<T> type '{type.FullName}'!");
                        IsWritableGenericIList = false;
                    }
                }
            }
            ...
            {
                Main.LogWarning($"Exception processing IEnumerable for IList<T> check: {ex.ReflectionExToString()}");
                IsWritableGenericIList = false;
            }
        }
        ...
        public void TrySetValueToIndex(object value, int index)
        {
            ...
            {
                Main.LogWarning($"Exception setting IList value: {ex}");
            }
        }
